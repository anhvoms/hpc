...
#
# This script is adapted from shipyard (github.com/Azure/batch-shipyard)
#
set -e
set -o pipefail
...
setup_glusterfs() {
    ...
    set +e
    for ip in "${peers[@]}"; do
        local host=${hostname_prefix}$(seq -f "%03g" $i $i)
        hosts=("${hosts[@]}" "$host")
...
# begin processing
while getopts "h?abc:d:f:i:lm:no:pr:s:t:" opt; do
    case "$opt" in
        h|\?)
            echo "glusterfs_setup.sh parameters"
            echo ""
            echo "-a attach mode"
            ...
            echo "-l [disks layout only] layout the brick only, not adding to gluster"
